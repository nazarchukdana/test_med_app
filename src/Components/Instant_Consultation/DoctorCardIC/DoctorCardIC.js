...
const DoctorCardIC = ({ name, speciality, experience, ratings, profilePic }) => {
 ...
 const dispatchAppointmentEvent = (type) => {
   ...
   const handleCancel = (appointmentId) => {
    ...
    let allAppointments = JSON.parse(localStorage.getItem('allAppointmentsIC')) || [];
   allAppointments = allAppointments.filter(
      (app) => !(app.user === user && app.appointment.id === appointmentId)
    );
    localStorage.setItem('allAppointmentsIC', JSON.stringify(allAppointments)); // Remove the appointment for this doctor
  ...
  const handleFormSubmit = (appointmentData) => {
    ...
    localStorage.setItem('allAppointmentsIC', JSON.stringify(allAppointments));

    // Dispatch the event to notify others of the new appointment
    dispatchAppointmentEvent('create');
  };
useEffect(() => {
    const allAppointments = JSON.parse(localStorage.getItem('allAppointmentsIC')) || [];
